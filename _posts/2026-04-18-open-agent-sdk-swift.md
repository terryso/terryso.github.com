...
Open Agent SDK 是一个用 Swift 6.1 编写的开源 Agent SDK，要求 macOS 13+。它的核心理念是：**在进程内运行完整的 Agent Loop**，用原生 Swift 并发（async/await、AsyncStream）来驱动整个 LLM 交互循环——发送提示、解析响应、执行工具调用、将结果反馈回 LLM，直到获得最终答案。

项目灵感来自 [open-agent-sdk-typescript](https://github.com/codeany-ai/open-agent-sdk-typescript)，将同样的 Agent 架构带到了 Swift 生态。同系列还有 Go 版本。
...
就这么简单。`prompt()` 是阻塞式的，一次调用完成整个 Agent Loop。如果你需要流式输出，可以用 `stream()`：
...
这是这个 SDK 最让人印象深刻的特性之一。它不是只提供了一个简单的 LLM 调用封装，而是内置了一整套 Agent 工具：
...
这些工具覆盖了代码读写、文件搜索、网页抓取、任务管理、团队协作、定时任务、LSP 集成等几乎所有 Agent 需要的能力。
...
工具的定义遵循标准的 JSON Schema 格式，输入会自动解码为 Swift 结构体。

## 多 LLM 提供商

除了默认的 Anthropic Claude，你可以切换到任何 OpenAI 兼容的 API：
...
Model Context Protocol (MCP) 是连接外部工具的标准协议。SDK 支持 stdio、SSE、HTTP 和进程内四种 MCP 传输方式：
...
这是更高级的特性。你可以让一个 Agent 生成子 Agent 来并行处理任务：
...
项目使用 Swift 6.1，通过 BMAD 方法论进行开发管理，拥有完整的测试套件和 E2E 测试。代码结构清晰，分为 API、Core、Hooks、MCP、Skills、Stores、Tools、Types、Utils 九个模块，约 90 个 Swift 源文件。

## 总结

Open Agent SDK (Swift) 为 Swift/macOS 生态带来了一套完整的 Agent 开发工具。它的设计哲学不是做一个简单的 LLM API 封装，而是提供一套**生产级别**的 Agent 运行时：完整的工具系统、多轮对话管理、会话持久化、权限控制、MCP 集成、子 Agent 协作——这些都是在构建真正可用的 AI Agent 应用时必不可少的基础设施。

如果你正在为 macOS 应用寻找 AI Agent 集成方案，或者对用 Swift 构建 Agent 系统感兴趣，值得一试。
